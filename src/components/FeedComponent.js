            ...
            setFeedarr(prevFeedArr =>
                prevFeedArr.map(feed => {
                    if (feed.pid === pid) {
                        ...
                        const updatedFeed = {...feed, likes: newLikes, liked: !feed.liked };
                ...
                {feedarr.map(feed => (
                    <div key={feed.pid} className="feed-item">
                        <div className="icon-container">
                            <i className="fas fa-user-circle"></i>&nbsp;<span>{" " + feed.pname}</span> <br /><br />
                        </div>
                        <img src={feed.image} alt={feed.pname} className="feed-image" />
                        <div className="feed-content">
                            <p className="feed-like">
                                <i className={`fas fa-heart ${feed.liked ? 'liked' : 'Not-liked'}`}onClick={() => updateLikes(feed.pid)}></i><span> {" " + feed.likes}</span>
                            </p>
                            <p className={`feed-status ${getStatusClass(feed.status)}`}>
                                <i className="fas fa-exclamation-circle"></i><span>{" " + feed.status}</span>
                            </p>
                            <p className="feed-location">
                                <i className="fas fa-map-marker-alt"></i><span>{" " + feed.location}</span>
                            </p>
                        </div>
                    </div>
                ))}
